...
public class BitGet : AnExchange
{
    ...
    protected override List<Product> GetProducts()
    {
        ...
        {
            var r = c.GetAsync($"{BASE_URL}/api/v2/spot/public/symbols").Result;
            if (r.StatusCode == HttpStatusCode.OK)
            {
                ...
                foreach (var p in data.EnumerateArray())
                {
                    try
                    {
                        ...
                        product.symbol = sym;
                        product.baseasset = p.GetProperty("baseCoin") + "";
                        product.quoteasset = p.GetProperty("quoteCoin") + "";
                        product.IsTradingEnabled = p.GetProperty("status").ToString() == "online";
    ...
    protected override List<Kline> GetLastKlines(string symbol)
    {
        ...
        using (HttpClient c = new())
        {
            ...
            var r = c.GetAsync($"{BASE_URL}/api/v2/spot/market/candles?symbol={symbol}&granularity=1min").Result;
            if (r.StatusCode == HttpStatusCode.OK)
            {
                var s = r.Content.ReadAsStringAsync().Result;
                JsonDocument j = JsonDocument.Parse(s);
